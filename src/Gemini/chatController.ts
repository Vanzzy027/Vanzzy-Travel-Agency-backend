...
export const handleChat = async (c: Context) => {
  try {
    ...
    if (calls && calls.length > 0) {
      ...
      console.log(
        `[AI] Triggering function: ${functionName} for user ${user.user_id}`,
      );
      ...
      if (functionName === "check_availability") {
        ...
        if (!args.vehicle_id || !args.days || !args.start_date) {
          return c.json(
            {
              error:
                "Missing required fields: vehicle_id, days, and start_date are all required",
            },
            400,
          );
        }

        functionResult = await toolsFunctions.create_booking(
          args,
          user.user_id,
          authHeader || "",
        );
