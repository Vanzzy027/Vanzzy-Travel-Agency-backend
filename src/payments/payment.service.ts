    ...
    try {
      ...
      `;
        ...
        paymentRequest.input(
          "commission_fee",
          completePaymentData.commission_fee,
        );
        paymentRequest.input("net_amount", completePaymentData.net_amount);
        paymentRequest.input(
          "payment_method",
          completePaymentData.payment_method,
        );
        paymentRequest.input(
          "payment_status",
          completePaymentData.payment_status,
        );
        paymentRequest.input(
          "transaction_id",
          completePaymentData.transaction_id || "",
        );
        paymentRequest.input(
          "transaction_reference",
          completePaymentData.transaction_reference || "",
        );
      ...
      `;
        ...
        const bookingDataResult =
          await getBookingRequest.query(getBookingQuery);

        await transaction.commit();

        console.log(
          `✅ Payment saved and booking ${completePaymentData.booking_id} status updated to 'Confirmed'`,
        );
