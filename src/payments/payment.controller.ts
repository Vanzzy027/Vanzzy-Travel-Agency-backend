...
export class PaymentController {
        ...
        return c.json(
          {
            success: false,
            message:
              "Please provide either paymentId as URL parameter or bookingId as query parameter",
          },
          400,
        );
      }

      const receiptData = await this.paymentService.getReceiptData(
  ...
  async getPaymentByBooking(c: Context) {
    try {
      ...
      const payment = await this.paymentService.getLatestPaymentByBooking(
        parseInt(bookingId),
      );
  ...
  async downloadReceipt(c: Context) {
    try {
      ...
      c.header(
        "Content-Disposition",
        `attachment; filename="receipt-${paymentId}.json"`,
      );
