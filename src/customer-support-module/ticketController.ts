...
export const getAllTickets = async (c: Context) => {
  try {
    ...
    return c.json(
      { error: "Failed to fetch all tickets", details: error.message },
      500,
    );
  }
};
...
export const getTicketById = async (c: Context) => {
  try {
    ...
    if (!idParam) return c.json({ error: "Ticket ID is required" }, 400);
...
export const updateTicketStatus = async (c: Context) => {
  try {
    ...
    const result = await ticketService.updateTicketStatus(id, {
      status,
      admin_response,
    });
