...
const shouldTrustCertificate =
  SQL_TRUST_SERVER_CERTIFICATE === "true" && !isAzureServer;

console.log(`🌍 Environment: ${process.env.NODE_ENV || "development"}`);
console.log(
  `🔗 Connecting to: ${isAzureServer ? "Azure" : "Local"} SQL Server`,
);
...
const initDatabaseConnection = async () => {
  ...
  try {
    ...
    console.log(
      `🔧 Using port: ${sqlConfig.port}, Encrypt: ${sqlConfig.options.encrypt}`,
    );
...
export const getDbPool = (): sql.ConnectionPool => {
  if (!globalPool || !globalPool.connected) {
    throw new Error(
      "Database not connected. Call initDatabaseConnection() first.",
    );
  }
  return globalPool;
};
