...
// --- HELPER FUNCTIONS ---
export const checkVehicleAvailability = async (vehicle_id, booking_date, return_date) => {
    try {
...
export const calculateTotalAmount = async (vehicle_id, booking_date, return_date) => {
    try {
        const pool = await getDbPool();
...
export const getUserBookingsService = async (user_id, filters) => {
    try {
        const pool = await getDbPool();
        let query = `
      SELECT b.*, u.first_name as user_first_name, u.last_name as user_last_name, u.email as user_email, u.contact_phone as user_contact_phone,
...
export const getVehicleBookingsService = async (vehicle_id, filters) => {
    try {
        const pool = await getDbPool();
        let query = `
      SELECT b.*, u.first_name as user_first_name, u.last_name as user_last_name, u.email as user_email, u.contact_phone as user_contact_phone,
...
export const updateBookingStatusService = async (bookingId, newStatus) => {
    ...
    if (!currentBooking.recordset.length)
...
export const cancelBookingService = async (id, user_id) => {
    try {
        const pool = await getDbPool();
...
export const completeBookingService = async (id, data) => {
    try {
        const pool = await getDbPool();
    ...
    `;
        ...
        if (result.recordset.length === 0)
            return null;
        const completedBooking = result.recordset[0];
...
// --- PRIVATE HELPERS ---
const updateVehicleStatus = async (vehicle_id, status) => {
    try {
        const pool = await getDbPool();
        await pool.request()
            .input('vehicle_id', sql.Int, vehicle_id)
...
const updateVehicleStatusAndMileage = async (vehicle_id, status, mileage) => {
    try {
        const pool = await getDbPool();
