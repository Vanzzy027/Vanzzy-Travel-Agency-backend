import { createBookingService, getAllBookingsService, getBookingByIdService, updateBookingService, updateBookingStatusService, // ✅ IMPORT THE NEW SERVICE
cancelBookingService, getUserBookingsService, getVehicleBookingsService, completeBookingService } from "./bookings.service.js";
...
export const createBooking = async (c) => {
    try {
        ...
        const isAvailable = await bookingService.checkVehicleAvailability(bookingData.vehicle_id, bookingData.booking_date, bookingData.return_date);
        if (!isAvailable)
            return c.json({ error: "Vehicle is not available for the selected dates" }, 400);
...
export const updateBookingStatusController = async (c) => {
    try {
...
export const getVehicleBookings = async (c) => {
    try {
...
// Complete booking
export const completeBooking = async (c) => {
    try {
        const id = parseInt(c.req.param('id'));
        if (isNaN(id) || id <= 0)
            return c.json({ error: "Invalid booking ID" }, 400);
