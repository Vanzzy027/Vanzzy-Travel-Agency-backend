...
const searchAvailableVehicles = async (searchQuery) => {
    try {
        ...
        return allAvailable.filter(vehicle => vehicle.manufacturer?.toLowerCase().includes(queryLower) ||
...
export const toolsFunctions = {
    ...
    check_availability: async ({ searchQuery }) => {
        try {
            ...
            const vehicleListText = formatted.map(v => `ID: ${v.id} - ${v.name} (${v.type}, ${v.color}, ${v.seating_capacity} seats) - $${v.price}/day`).join('\n');
            return JSON.stringify({
                success: true,
    ...
    create_booking: async ({ vehicle_id, days, start_date }, userId, authHeader) => {
        try {
