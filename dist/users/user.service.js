...
//update by id 
export const updateUserByIdService = async (user_id, updateData) => {
    const db = getDbPool();
    const allowedFields = {
...
// Store OTP Service 
export const storeOTPService = async (email, otp) => {
    try {
        const pool = await getDbPool();
        const query = `
      UPDATE Users 
...
export const updatePasswordService = async (email, hashedPassword) => {
    try {
        const pool = await getDbPool();
        const query = `
      UPDATE Users 
...
export const changeUserRoleService = async (user_id, role) => {
    try {
        const pool = await getDbPool();
        const query = `
      UPDATE Users 
