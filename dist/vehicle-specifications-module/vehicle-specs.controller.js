import { createVehicleSpecService, getAllVehicleSpecsService, getVehicleSpecByIdService, updateVehicleSpecService, deleteVehicleSpecService, getVehicleSpecsByTypeService } from "./vehicle-specs.service.js";
import { VehicleSpecSchema } from "../validators/vehicle.validators.js";
// Create new vehicle specification
export const createVehicleSpec = async (c) => {
    try {
...
export const getVehicleSpecById = async (c) => {
    try {
...
export const updateVehicleSpec = async (c) => {
    try {
        const id = parseInt(c.req.param('id'));
        if (isNaN(id) || id <= 0) {
            return c.json({ error: "Invalid vehicle specification ID" }, 400);
...
export const deleteVehicleSpec = async (c) => {
    try {
        const id = parseInt(c.req.param('id'));
        if (isNaN(id) || id <= 0) {
            return c.json({ error: "Invalid vehicle specification ID" }, 400);
